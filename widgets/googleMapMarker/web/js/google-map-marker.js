function GoogleMapMarkerManager()
{
    ...
    const widgetSelector = '.google-map-marker-input-widget';
    ...
    var initializeWidget = function ( widgetContainer )
    {
        ...
        {
            var widget = new GoogleMapMarkerWidget(widgetContainer);
            widget.initialize();
            return widget;
        }
        return null;
...
function GoogleMapMarkerWidget ( widget )
{

    const inputSelector = 'input.google-map-marker-input-widget-input';

    const searchBarSelector = 'input.google-map-marker-input-widget-search-bar';

    const canvasSelector = 'div.google-map-marker-input-widget-canvas';
    ...
    var initializeMap = function()
    {
        ...
        if($(widget).data('has-coordinates') == 1) {
            var marker = new google.maps.Marker({
                position: getInitialMapCenter(),
                map: map,
                title: $(widget).data('current-marker-label')
            });
        }
    ...
    var makePointString = function ( pointData )
    {
        ...
        $($(widget).data('latitude-input-selector')).attr('value', point.lat());
        $($(widget).data('longitude-input-selector')).attr('value', point.lng());
...
(
    function()
    {
        ...
        googleMapMarkerManager = new GoogleMapMarkerManager();

        // Initialize widgets
        googleMapMarkerManager.initializeWidgets();

        $('.google-map-marker-input-widget-search-bar').on('keyup, keydown, keypress',function(evt){
